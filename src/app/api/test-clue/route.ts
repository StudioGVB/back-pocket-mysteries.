...
export async function GET(request: Request) {
  ...
  try {
    ...
    const isAudio = clue.title?.toLowerCase().includes('audio') || clue.description?.toLowerCase().includes('audio');
    const isCctv = clue.title?.toLowerCase().includes('cctv') || clue.description?.toLowerCase().includes('cctv');
    const isScreen = true; // Milo's Final Draft is a message draft
